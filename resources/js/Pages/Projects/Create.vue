<script setup>
    ...
    const submitForm = () => {
        ...
        // ✅ Ensure proper date format before sending
        form.transform(data => ({
            ...data,
...
<template>
    ...
    <button v-if="authUser.role === 'Program Manager'"
        type="button"
        class="py-3 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent
               bg-gray-800 text-white hover:bg-cyan-600 focus:outline-none focus:bg-cyan-600 disabled:opacity-50
               disabled:pointer-events-none"
        aria-haspopup="dialog"
        aria-expanded="false"
        aria-controls="modal-create-project"
